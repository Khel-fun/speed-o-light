...
export const appRouter = router({
  ...
  newGame: publicProcedure
    .input(z.object({ playerAddress: z.string().min(1) }))
  ...
  submitSession: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const { sessionId, playerAddress, tapSequence, dangerTap } = input;
      ...
      for (let i = 0; i < 136; i++) {
        if (tapSequence[i].is_danger !== gridSequence[i].is_danger) {
          throw new TRPCError({
            code: "BAD_REQUEST",
      ...
      if (dangerTap.seq_pos !== "255") {
        ...
        if (!gridSequence[dangerPos].is_danger) {
          throw new TRPCError({
            code: "BAD_REQUEST",
      ...
      const circuit = await prisma.circuits.findFirstOrThrow({
        where: { circuit_name: "speed_o_light_game_state" },
      });
      ...
      return { success: true, score, xp, isWinner };
    }),
